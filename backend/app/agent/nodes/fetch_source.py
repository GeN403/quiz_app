...
def fetch_source(state: AgentState) -> dict[str, Any]:
    ...
    if len(text) > 8000:
        logger.info(f"[fetch_source] Text truncated from {len(text)} to 8000 chars")
        text = text[:8000]
    ...
    if selected_quote:
        ...
        if is_found:
            ...
            logger.info("[fetch_source] Selected quote not found, using first candidate")
            selected_quote_final = quotes[0] if quotes else ""
    else:
        selected_quote_final = quotes[0] if quotes else ""

    logger.info(f"[fetch_source] Success: title={title!r}, text_len={len(text)}")
    return {
        "source_text": text,
        "source_title": title,
